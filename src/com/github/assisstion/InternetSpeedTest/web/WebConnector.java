...
public class WebConnector{
	 ...
	 * @throws IOException
	 *             when the webpage cannot be connected to
	 */
	 ...
	 * @return a long array with two values: the number of bytes of the website
	 *         and the ms passsed
	 * @throws IOException
	 *             when the webpage cannot be connected to
	 */
	public static Pair<Long, Long> webpageByteCount(URL url, boolean silent,
			MainGUI gui,
			Collection<InfoSender<Pair<Pair<Long, Long>, Integer>>> senders,
			int attemptCount, LifeLine lifeLine) throws IOException{
		WebpageByteCount wbc = new WebpageByteCount(url, silent, gui, senders,
				attemptCount, lifeLine);
		wbc.execute();
		try{
			return wbc.get();
		}
	...
	public static class WebpageByteCount extends
		...
		public WebpageByteCount(
				URL url,
				boolean silent,
				MainGUI gui,
				Collection<InfoSender<Pair<Pair<Long, Long>, Integer>>> senders,
				int attemptCount, LifeLine lifeLine){
			this.url = url;
			this.silent = silent;
			this.gui = gui;
		...
		protected Pair<Long, Long> doInBackground() throws Exception{
			...
			while(value >= 0){
				count++;
				if(count % 1000 == 0){
					publish(new Pair<Long, Long>(count,
							System.currentTimeMillis()));
				}
				if(count % 1000000 == 0){
					value = readWithTimeOut(bis, 5000);
					if(!silent){
						System.out.println("Counted " + count / 1000000 +
								" million.");
					}
					if(count >= 100000000){
						break;
		...
		protected void process(List<Pair<Long, Long>> pairs){
			...
			gui.speed.setText(String.valueOf(MathHelper
					.roundThreeDecimals((double) bytes / (double) time)));
			for(InfoSender<Pair<Pair<Long, Long>, Integer>> sender : senders){
				sender.send(new Pair<Pair<Long, Long>, Integer>(
						new Pair<Long, Long>(bytes, time), attemptCount));
			}
	 ...
	 * @return
	 */
	public static int readWithTimeOut(InputStream is, long timeOut)
			throws IOException{
		TimedReader reader = new TimedReader(is);
		TimeOutRunner<Integer> runner = new TimeOutRunner<Integer>(reader,
				timeOut);
		reader.setRunner(runner);
		return runner.get();
	}

	public static InputStream loadWithTimeOut(URL url, long timeOut)
			throws IOException{
		TimedURLLoader reader = new TimedURLLoader(url);
		TimeOutRunner<InputStream> runner = new TimeOutRunner<InputStream>(
				reader, timeOut);
		reader.setRunner(runner);
		return runner.get();
	}
		...
		public T get() throws IOException{
			...
			while(System.currentTimeMillis() - startTime < timeOut){
				synchronized(this){
					...
					if(toBeThrown != null){
						throw new IOException("Exception caught while loading",
								toBeThrown);
					}
					if(value != null){
						return value;
